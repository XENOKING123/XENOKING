              ...
              <div className="flex items-center gap-2 font-medium">
                ...
                {mobile
                  ? tr(
                      ...
                      undefined,
                      "Keep the screen on while XENO TOOL is in the foreground",
                    )
                  : tr(
                      "keep_awake_on",
                      undefined,
                      "Keep the computer awake while XENO TOOL is open",
                    )}
              </div>
...
function ThemePicker() {
  ...
  // XENOKING first (the default), then 29 more — 30 swatches in a grid. The
  // sidebar palette button has all 50.
  const list = THEMES.slice(0, 30);
  return (
    <div
      ...
      {list.map((t) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col gap-1.5 rounded-lg border-2 p-2 transition ${
              active
          ...
          >
            <span
              ...
              style={{
                background: `linear-gradient(135deg, ${t.swatch}, ${t.swatch} 35%, transparent)`,
                boxShadow: `0 0 14px -4px ${t.swatch}`,
              }}
            />
            <span className="flex items-center justify-between gap-1">
              <span className="truncate text-[11px] font-semibold">{t.label}</span>
              {active && <CheckCircle2 size={12} className="shrink-0 text-[var(--color-good)]" />}
            </span>
          </button>
        );
      })}
...
function ResetPanel() {
  ...
  async function doReset() {
    const ok = await confirm({
      title: tr("reset_confirm_title", undefined, "Reset XENO TOOL?"),
      message: tr(
        "reset_confirm_body",
        undefined,
        "This permanently deletes ALL local XENO TOOL data — settings, your saved PS5s, activity/queue history, payload caches, and crash reports. It does NOT touch anything on your PS5. The app reloads to a fresh state. This can't be undone.",
      ),
    ...
    <div className="space-y-3 text-sm">
      <p className="text-xs text-[var(--color-muted)]">
        {tr(
          "reset_hint",
          undefined,
          "Wipe every local trace of XENO TOOL and start over: settings, saved PS5s, activity/queue history, payload caches, and crash reports. Nothing on your PS5 is affected. Export a backup above first if you might want it back.",
        )}
      </p>
      <button
        type="button"
      ...
      {phase.kind === "downloaded" && (
        <div className="rounded-md border border-[var(--color-good)] bg-[var(--color-surface)] p-3 text-xs">
          ...
          <ol className="list-inside list-decimal space-y-0.5 text-[var(--color-muted)]">
            <li>{tr("update_step_quit", undefined, "Quit XENO TOOL.")}</li>
...
function pickInstallHint(
  ...
  if (lower.endsWith(".dmg")) {
    return tr(
      "install_hint_dmg",
      undefined,
      "Double-click the .dmg, drag XENO TOOL into Applications, replace when asked.",
    );
  }
  if (lower.endsWith(".zip")) {
    return tr(
      "install_hint_zip",
      undefined,
      "Extract the zip and replace the existing XENO TOOL with the new one.",
    );
  }
  return tr(
    "install_hint_generic",
    undefined,
    "Replace the existing XENO TOOL with the newly downloaded file.",
  );
}
