...
import { usePayloadFavs } from "../../state/payloadFavs";
...
export default function CatalogPanel() {
  ...
  const favs = usePayloadFavs((s) => s.favs);
...
function PayloadCard({
  ...
  const tr = useTr();
  const isFav = usePayloadFavs((s) => s.favs.includes(info.id));
  const toggleFav = usePayloadFavs((s) => s.toggle);
      ...
      <div className="flex flex-wrap items-start gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => toggleFav(info.id)}
              title={isFav ? "Unfavorite" : "Favorite"}
              className={isFav ? "text-[var(--color-gold)]" : "text-[var(--color-muted)] hover:text-[var(--color-gold)]"}
            >
