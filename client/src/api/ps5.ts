...
export function humanizeJobErrorReason(reason: string | undefined): string | null {
  ...
  switch (reason) {
    ...
    case "packed_unsupported":
      return "The PS5 helper rejected a packed transfer — this happens with an outdated payload when a folder upload comes down to a single small file (e.g. a Resume of a game with many tiny files). Update XENO TOOL to the latest version, re-send the payload from the Send Payload tab, then retry.";
    case "size_mismatch":
