...
// Never STORE these as the resume target.
const SKIP_RESTORE_ROUTES = new Set(["/first-run", "/whats-new", "/", "/home"]);
// Only RESTORE a stored route when landing on these paths. Crucially NOT "/":
// a fresh boot lands on the Home showcase (via LandingRedirect) and must NOT
// be hijacked by a stale last-route — the user expects Home every launch.
const RESTORE_FROM_ROUTES = new Set(["/whats-new"]);
...
function useRoutePersistence() {
  ...
  useEffect(() => {
    ...
    if (!RESTORE_FROM_ROUTES.has(location.pathname)) return;
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-[var(--color-muted)]">
              {tr(
                ...
                undefined,
                "Grant All files access so XENO TOOL can upload game folders, .zip dumps, and .pkg files from your phone.",
              )}
            </p>
          </div>
        </div>
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--color-surface)] text-[var(--color-text)]">
      ...
      <div className="flex items-center gap-2 border-b border-[var(--color-border)] bg-[var(--color-surface-2)] px-3 pb-2 pt-[calc(env(safe-area-inset-top)_+_0.5rem)] md:hidden">
        ...
        />
        <span className="xeno-wordmark text-base font-black tracking-[0.12em]">XENO TOOL</span>
      </div>
