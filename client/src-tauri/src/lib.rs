...
pub fn run() {
    ...
    let app = builder
        ...
        .setup(|app| {
            ...
            // First-run: unpack the bundled trainer library (~9k) + title
            // catalog into the data dir so a fresh install has every cheat and
            // a name for almost every title immediately. Idempotent (stamped).
            commands::seed_on_startup(app.handle());
