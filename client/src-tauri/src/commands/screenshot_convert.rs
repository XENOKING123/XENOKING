...
// Stub used by the default build (jpegxr/libclang not compiled in). Keeps the
// `screenshot_convert` command present so the frontend invoke never breaks; it
// just reports that HDR conversion isn't in this build.
#[cfg(all(not(any(target_os = "android", target_os = "ios")), not(feature = "screenshot-jxr")))]
mod desktop {
    pub fn convert_jxr_to_png(_jxr: &[u8]) -> Result<Vec<u8>, String> {
        Err("HDR .jxr → PNG conversion isn't enabled in this XENO build. \
             (Rebuild with --features screenshot-jxr, which needs LLVM, to enable it.)"
            .into())
    }
}

#[cfg(all(not(any(target_os = "android", target_os = "ios")), feature = "screenshot-jxr"))]
mod desktop {
