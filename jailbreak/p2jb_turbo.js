...
(async function () {
    try {
        ...
        async function prepare_fds(S) {
            ...
            await ulog("TURBO: " + NW + " leak workers on cores [" +
                LEAK_CORES.join(",") + "]");
            await ulog("TURBO: ~" + (Number(TOTAL_SYSCALLS) / 1e9).toFixed(1) +
                "B syscalls total, ~" +
                (Number(TOTAL_SYSCALLS / BigInt(NW)) / 1e9).toFixed(1) +
                "B per worker");
            ...
            while (!all_fed) {
                ...
                if (now - last_report >= 30000) {
                    last_report = now;
                    let fed_total = 0n;
                    for (const lw of lws) fed_total += lw.queued;
                    const pct = Number(fed_total * 100n / total_batches);
                    if (pct > last_pct) {
                        last_pct = pct;
                        const elapsed_s = (now - leak_start) / 1000;
                        const eta_s = pct > 0
                            ? Math.round(elapsed_s * (100 - pct) / pct) : 0;
        ...
        switch (leak_nw) {
            ...
            case 6: eta_minutes = 33; break;
            case 7: eta_minutes = 28; break;
            case 8: eta_minutes = 24; break;
        ...
        await ulog("=== P2JB TURBO complete ===");
        send_notification("P2JB TURBO\nJailbreak complete!\nFW: " + FW_VERSION);
